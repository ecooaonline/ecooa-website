...
| PROM-09 | Conversão dispara de forma confiável em /obrigado | revisão 2026-06-13 | P14 (coord. P7/consent) | (a) na página de conversão, após consentimento aceito, forçar `loadGTM()`/`loadMetaPixel()` imediatamente (exceção legítima ao interaction-gating do BaseLayout) para o `dataLayer.push({event:'form_submit_success'})` drenar e o Meta `Lead` disparar; (b) remover o bloco `window.gtag(...)` morto em `obrigado.astro` (site usa GTM, não gtag.js); (c) só marcar o guard `sessionStorage` do token após confirmar carga do tracker; (d) garantir trigger `form_submit_success` no container GTM (painel do dono) | aberta |
