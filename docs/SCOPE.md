...
| HIP-09 | Funil do formulário pós-CP-01 funciona em produção de ponta a ponta | implementação + código GAS | conversão secundária quebrada sem sinal | dono (teste real) | P4 (gate ativo) | **aberta — gate do P4 ATIVO (2026-06-14). Protocolo de Teste definido em `CONVERSION_GOVERNANCE.md` §8; itens 1/3/4/5 verificáveis em preview pela IA, item 2 (chegada do lead ao destino) é do dono. Risco interino aceito 2026-06-12. P04 fecha com esta pendência formal nomeada ao dono.** |
...
| PROM-07 | Leads de preview não contaminam produção | P3 | P4 | gate de host em `form-submit.ts`: fora do canônico, bloquear envio real com aviso + fallback WhatsApp (ou `_source=preview`); risco interino aceito pelo dono (2026-06-12) | **renegociada no P4 (2026-06-14): mecanismo segue code-ready, mas implementar o host-gate ANTES de validar o funil real (HIP-09) é instrumentar estanqueidade sobre um fluxo não confirmado. Devedora segue P4, BLOQUEADA pelo gate HIP-09; implementar + re-testar (Lei de Ferro) após o dono confirmar o E2E. Risco interino já aceito.** |
| PROM-08 | Match operante em previews CF | P3 | P4 | incluir `*.workers.dev` (preview) na allowlist de attestation do match (`match.astro:497-499`), mantendo safe-mode para domínios estranhos | **renegociada no P4 (2026-06-14): depende do cutover Cloudflare (preview ainda é Vercel; P03 deixou só o esqueleto). Implementar quando o preview `*.workers.dev` existir de fato. Sem impacto na conversão.** |
| PROM-09 | Conversão dispara de forma confiável em /obrigado | revisão 2026-06-13 | P14 (coord. P7/consent) | (a)✅ /obrigado dispara `ecooa_force_analytics`; BaseLayout carrega GTM+Pixel imediatamente após consentimento (exceção da página de conversão); (b)✅ `window.gtag` morto removido; (c)✅ evento enfileirado no dataLayer; **(d) PENDENTE — dono: criar trigger `form_submit_success` no container GTM mapeado para a conversão GA4/Ads** | parcial (código feito 2026-06-13; falta painel do dono) |
| PROM-10 | Trigger de conversão no GTM (`form_submit_success`) | P4 | P14 (+ painel do dono) | a página já empurra `form_submit_success` no dataLayer; falta criar no container GTM o trigger de evento personalizado → tag de conversão GA4/Ads. Anexo: `EVENTS_TRACKING_PLAN.md`. **Consolida PROM-09(d).** | aberta |
| PROM-11 | Consent Mode v2 granular respeitado por toda tag | P4 | P14 | GTM/Pixel hoje são interaction-gated + força em /obrigado com consentimento aceito; falta consolidar Consent Mode v2 granular por categoria (`DATA_GOVERNANCE.md` §3) | aberta |
