...
(function () {
  ...
  /* A busca NAO fica na URL. O tribunal etico apontou o defeito que eu havia
     criado: a frase clinica literal do visitante ia para a barra de endereco e
     para o historico do navegador, num aparelho que muitas vezes e
     compartilhado, e a propria tela promete que a frase nao e armazenada.
     Links de entrada com ?q= continuam funcionando, para campanha e para o
     SearchAction, mas a URL e limpa assim que a busca roda. */
  function limpaURL() {
    if (!window.history || !window.history.replaceState || !location.search) return;
    try { window.history.replaceState(null, '', location.pathname); } catch (e) { /* file:// */ }
  }
