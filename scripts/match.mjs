...
(function () {
  ...
  var SINTOMAS = [
    ...
    { id: 'integrativa', rotulo: 'terapias integrativas', area: 'saude-integrativa',
      ...
      termos: ['integrativa', 'integrativo', 'holistico', 'holistica', 'biorressonancia', 'terapia complementar', 'acupuntura', 'auriculoterapia', 'reiki', 'aromaterapia', 'enxaqueca', 'dor de cabeca', 'cefaleia', 'soroterapia', 'soro de vitaminas'] },
    ...
    { id: 'pele-clinica', rotulo: 'pele e rosto', area: 'estetica-facial',
      ...
      termos: ['pele', 'acne', 'espinha', 'espinhas', 'cravos', 'mancha', 'manchas', 'melasma', 'rosto', 'facial', 'face', 'dermatologista', 'dermatologia', 'skincare', 'poro', 'poros', 'peeling', 'melanose', 'rosacea', 'dermatite', 'cicatriz de acne'] },
    { id: 'harmonizacao', rotulo: 'harmonização facial', area: 'estetica-facial',
      ...
      termos: ['ruga', 'rugas', 'linhas de expressao', 'botox', 'toxina botulinica', 'preenchimento', 'acido hialuronico', 'harmonizacao facial', 'harmonizacao', 'hof', 'rejuvenescimento', 'flacidez facial', 'olheira', 'olheiras', 'labios', 'labio', 'bioestimulador'] },
    ...
    { id: 'materno-infantil', rotulo: 'nutrição materno-infantil', area: 'nutricao',
      ...
      termos: ['nutricao na gravidez', 'nutricao para gestante', 'nutricao infantil', 'nutricionista infantil', 'gestante', 'gravida', 'gestacao', 'gravidez', 'tentante', 'introducao alimentar', 'crianca', 'criancas', 'infantil', 'seletividade alimentar', 'meu filho nao come', 'lactacao'] },
    ...
    { id: 'psico-esporte', rotulo: 'psicologia do esporte', area: 'saude-mental',
      ...
      pros: ['augusto-kauer'],
      termos: ['psicologia do esporte', 'psicologo do esporte', 'psicologa do esporte', 'psicologia esportiva', 'pressao em competicao', 'ansiedade pre competicao', 'mentalidade de atleta', 'desempenho mental'] },
  ...
  /* card do almanaque: foto pequena focada no rosto (object-position no topo,
     para nunca cortar a cabeça), nome, profissão e registro, e o texto do que
     este profissional faz para este bloco de queixa. */
  function cardResultado(p, grupo, i) {
    ...
    var moldura = el('span', 'display:block; flex:0 0 clamp(72px,9vw,96px); width:clamp(72px,9vw,96px); aspect-ratio:1; position:relative; overflow:hidden; background:#E9E7E2;');
    if (p.foto) {
    ...
    corpo.appendChild(el('span', 'display:block; margin-top:10px; max-width:64ch; font-size:14px; line-height:1.66; color:#66645E;', [texto]));
    ...
    ag.href = 'https://wa.me/' + WA + '?text=' + encodeURIComponent(msg);
  ...
  function renderResultado() {
    ...
    if (visiveis.length) {
      ...
      var lista = el('div', 'display:grid; grid-template-columns:1fr; gap:1px; max-width:1040px; border-top:1px solid #DEDCD6; border-bottom:1px solid #DEDCD6; background:#FAF9F7;');
      visiveis.forEach(function (p, i) { lista.appendChild(cardResultado(p, grupo, i)); });
      faixa.appendChild(lista);
      if (pool.length > LIMITE || (grupo.id === 'perguntas' && grupo.area)) {
        ...
        mais.textContent = 'ver todos os profissionais desta área →';
